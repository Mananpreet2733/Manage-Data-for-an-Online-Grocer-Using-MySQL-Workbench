...
## Key Features
### VENDORS Table:
Captures essential supplier information, each identified by a unique vendor ID.

### PRODUCTS Table: 
Details each item sold, including its description, type, and unit of measurement. Links each product to its supplier through a foreign key.
### INVENTORY Table:
Tracks stock levels, costs, purchase dates, and storage locations. References products by their unique item numbers.
### SALES Table:
Records sales transactions, specifying the product sold, sale date, customer, quantity, and sale price. Maintains a reference to product item numbers.
...
## Dataframe Creation:
Separate DataFrames for vendors, products, inventory, and sales were created, ensuring unique IDs and organized data ready for database insertion.
## Saving Processed Data:
The cleaned data was saved as CSV files, facilitating easy import into the MySQL database.
